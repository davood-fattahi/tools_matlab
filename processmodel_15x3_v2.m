function y=processmodel_15x3_v2(x,delta,beta,theta,w)
...
y(1)=mod(x(1)+x(3)*delta,2*pi)+w(1);

%%% p wave
dc(1)=rem((x(1)-theta(11)+pi),2*pi)-pi; % distance from the center
ep(1)=exp(-(dc(1)^2)./(2*theta(6)^2)); % Gaussian function (differential form) 
zp=(delta*theta(1)*x(3))*dc(1)*(ep(1))./(2*pi*theta(6)^3); % scaling the Gaussian (differential form)


%%% Q wave
dc(2)=rem((x(1)-theta(12)+pi),2*pi)-pi;
ep(2)=exp(-(dc(2)^2)./(2*theta(7)^2));
zq=(delta*theta(2)*x(3))*dc(2)*(ep(2))./(2*pi*theta(7)^3);

%%% R wave
dc(3)=rem((x(1)-theta(13)+pi),2*pi)-pi;
ep(3)=exp(-(dc(3)^2)./(2*theta(8)^2));
zr=(delta*theta(3)*x(3))*dc(3)*(ep(3))./(2*pi*theta(8)^3);

%%% S wave
dc(4)=rem((x(1)-theta(14)+pi),2*pi)-pi;
ep(4)=exp(-(dc(4)^2)./(2*theta(9)^2));
zs=(delta*theta(4)*x(3))*dc(4)*(ep(4))./(2*pi*theta(9)^3);

%%% T wave
dc(5)=rem((x(1)-theta(15)+pi),2*pi)-pi;
ep(5)=exp(-(dc(5)^2)./(2*theta(10)^2));
zt=(delta*theta(5)*x(3))*dc(5)*(ep(5))./(2*pi*theta(10)^3);


y(2)=-(zp+zq+zr+zs+zt)+0.99*x(2)+w(2); % updating the synthetic ECG
y(3)=beta*x(3)+w(3); % updating the angular velocity
